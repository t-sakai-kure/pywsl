...
class PNU_AUC_SL(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, x, y):
        ...
        H_p, H_n, H_u = k_p.T.dot(k_p)/n_p, k_n.T.dot(k_n)/n_n, k_u.T.dot(k_u)/n_u
        h_p, h_n, h_u = np.sum(k_p, axis=0)/n_p, np.sum(k_n, axis=0)/n_n, np.sum(k_u, axis=0)/n_u
        print(h_p.shape)
        H_pn = H_p - h_p.T*h_n - h_n.T*h_p + H_n
        h_pn = h_p - h_n
        ...
        if self.eta >= 0: # PN+PU
            eta = self.eta
            k_pu = H_p - h_p.T*h_u - h_u.T*h_p + H_u
            k_pp = 2*H_p - 2*h_p.T*h_p
            H_xu = (k_pu - th_p*k_pp)/th_n
            h_xu = (h_p - h_u)/th_n
        else: # PN+NU
            eta = -self.eta
            k_nu = H_n - h_n.T*h_u - h_u.T*h_n + H_u
            k_nn = 2*H_n - 2*h_n.T*h_n
            H_xu = (k_nu - th_n*k_nn)/th_p
            h_xu = (h_u - h_n)/th_p

        self.etah_ = calc_etab(n_p, n_n, th_p)
        self.coef_ = sp.linalg.solve((1-eta)*H_pn + eta*H_xu + Reg,
                                     (1-eta)*h_pn + eta*h_xu)
        return self
...
def calc_etab(n_p, n_n, prior, eta):
